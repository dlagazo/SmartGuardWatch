...
public class LocationSensorService extends IntentService implements GoogleApiClient.ConnectionCallbacks,
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if(!mGoogleApiClient.isConnected()) {
            Log.d("Navigation", "Connecting");
            mGoogleApiClient.connect();
        } else {
            Log.d("Navigation", "Starting location updates");
    ...
    private void handleNewLocation(Location location) {
        ...
        prefs.edit().putString(Constants.PREFS_GPS_LAT, Double.toString(location.getLatitude())).apply();
        ...
        if(Utils.getCurrentTimeStampInSeconds() - editor.getLong(Constants.EMERGENCY_TIMER, 0) > 30) {
        ...
        testLoc.setLatitude(14.672427561084126);
        testLoc.setLongitude(121.02273523807526);
        Location testLoc1 = new Location("");
        testLoc1.setLatitude(14.67241199263442);
        testLoc1.setLongitude(121.0225635766983);

        Log.d(TAG, "Test distance: " + testLoc.distanceTo(testLoc1));
        */
